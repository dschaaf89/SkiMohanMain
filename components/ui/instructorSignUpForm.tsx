...
const InstructorSignupForm: React.FC<InstructorSignupFormProps> = ({
  programCode,
  onSubmit,
}) => {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <FormItem>
              ...
              <FormControl>
                <Input type="text" value={age !== null ? age.toString() : ""} readOnly />
              </FormControl>
            </FormItem>
            <FormField
                        control={form.control}
                    ...
                    return (
                      <FormItem className="flex flex-col space-y-4">
                        {classes.map((classTime) => (
                          <div key={classTime.id} className="flex flex-row items-start space-x-3">
                            <Checkbox
                              id={`class-time-${classTime.id}`}
                              checked={selectedClassTimes.includes(classTime.id)}
                              onCheckedChange={(checked) => {
                                if (checked) {
                                  field.onChange([...selectedClassTimes, classTime.id]);
                                } else {
                                  field.onChange(
                                    selectedClassTimes.filter((id) => id !== classTime.id)
                                  );
                                }
                              }}
                            />
                            <FormLabel className="font-normal">{classTime.label}</FormLabel>
                          </div>
                        ))}
                      </FormItem>
                    );
                              ...
                              >
                                <FormControl>
                                  <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                      if (checked) {
                                        field.onChange([...selectedClinics, item.id]);
                                      } else {
                                        field.onChange(
