...
const SummaryComponent = () => {
  ...
  // Check if any of the product codes are Instructor or Assistant
  const showPolicies = !items.some(
    (item) => item.name === "Instructor" || item.name === "Assistant"
  );
  ...
  const totalPrice = items.reduce((total, item) => {
    return total + Number(item.price) * item.quantity; // Multiply by quantity
  }, 0);

  const onCheckout = async () => {
    // Ensure policies are accepted before checkout
    if (showPolicies && (!acceptedRefundPolicy || !acceptedTeachingPhilosophy || !acceptedLatenessPolicy)) {
      toast.error("You must accept all policies before proceeding.");
      return;
    }
    ...
    try {
      ...
      // Retrieve program codes and store them in localStorage
        ...
        <div className="mt-4 space-y-4">
          <div className="flex items-center space-x-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => setAcceptedRefundPolicy(checked === true)} // Ensuring boolean value
            />
          ...
          <div className="flex items-center space-x-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => setAcceptedTeachingPhilosophy(checked === true)} // Ensuring boolean value
            />
            <span>I accept the Teaching Philosophy</span>
          </div>
          <div className="flex items-center space-x-3">
            <Checkbox
              checked={acceptedLatenessPolicy}
              onCheckedChange={(checked) => setAcceptedLatenessPolicy(checked === true)} // Ensuring boolean value
            />
            <span>I accept the Lateness Policy</span>
          </div>
        </div>
      )}

      <div className="flex justify-end mt-4">
        <Button onClick={onCheckout} disabled={!userId}>
          Proceed to Checkout
        </Button>
      </div>
    </div>
